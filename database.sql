...
CREATE TABLE IF NOT EXISTS `vendors_payment_log` (
  ...
  `order_id` int(11) NOT NULL,
  `channel` tinyint(3) unsigned NOT NULL DEFAULT '1' COMMENT '交易渠道(1 支付宝)',
  `trans_date` varchar(50) COMMENT '订单支付时间，格式为yyyy-MM-dd HH:mm:ss',
  `vendor_alipay_account` varchar(64) COMMENT '商户收款支付宝账号',
  `vendor_real_name` varchar(64) COMMENT '商户真实姓名',
  `tranfer_order_id` varchar(64) COMMENT '支付宝转账订单号',
  `out_biz_no` varchar(64) COMMENT '商户订单号',
  `pay_fund_order_id` varchar(50) COMMENT '支付宝支付资金流水号',
  `status` varchar(50) COMMENT '转账单据状态。 SUCCESS（该笔转账交易成功）：成功； FAIL：失败',
  `trans_amount` DOUBLE(20,2) DEFAULT 0 COMMENT '转账金额',
  `updated_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `code` varchar(50) COMMENT '网关返回码',
  `msg` varchar(50) COMMENT '网关返回码描述',
  `sub_code` varchar(50) COMMENT '业务返回码',
  `sub_msg` varchar(50) COMMENT '业务返回码描述',
  `comment` varchar(50) COMMENT '备注',
  PRIMARY KEY (`id`)
) ENGINE=MyISAM  DEFAULT CHARSET=utf8 AUTO_INCREMENT=1 ;

INSERT INTO `vendors_payment_log` (`id`, `vendor_id`, `order_id`, `trans_amountt`) VALUES
(1, 1, 24011892, 0.01);